...
for rotation in Images:
    if Masking[rotation]:
        print(f'Masking Storm 3-{rotation}...')
        #Load halo-level data
        Image = Images[rotation]
        Image = np.flip(Image,0)
        LogImage = plt.imread(f'Images/storm.3.{rotation}.png')

        #Find default isophote
        rbins = Profiles[rotation]['rbins']
        Rhalf = Profiles[rotation]['Reff']
        if np.isnan(Rhalf): Rhalf = Profiles[rotation]['Rhalf']
        ind_eff = np.argmin(abs(rbins-Rhalf))
        v = Profiles[rotation]['v_lum_den'][ind_eff]
        tol = .01
        iso = np.where((Image>v*(1-tol)) & (Image<v*(1+tol)))

        DC,DA,DE = False,False,False
        _VARS = InitializeGUI(f'Halo 3 - {rotation}')
        drawChart()
        while True:
            ...
            if event in ['Save','Ignore']:
                _VARS['window'].close()
                myprint(f'Saving Storm 3 - {rotation}...',clear=True)
                if event=='Ignore':iso=[[],[]]
                ba = SavePlot(LogImage,iso,Rhalf,f'Images/storm.3.{rotation}.Isophote.png')
                ShapeData[rotation] = ba
                pickle.dump(ShapeData,open(f'{sim}.{hid}.ShapeData.pickle','wb'))
                Masking[rotation] = False
                pickle.dump(Masking,open(f'{sim}.{hid}.Masking.pickle','wb'))
                myprint(f'Storm 3 - {rotation} saved.',clear=True)
                break
            if event=='Reset':
